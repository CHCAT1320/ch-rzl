...
function mixRGBAArray(color1, color2, ratio = 0.5) {
    // // 确保输入的颜色数组是有效的
    ...
    drawLine(tick){
        for (let i = 0; i < this.info.linePoints.length; i++) {
            ...
            if (lC !== null && Array.isArray(lC)){
                const mixColor = mixRGBAArray(lC, [color.r, color.g, color.b, color.a], 0.5);
...
class note {
    ...
    drawNote(tick){
        ...
        if (this.type === 2){
            ...
            const h = -cameraScale(tick) * (this.otherInformations[2] - this.fp) * cvs.height * (215 / 32 + speed) * (10 / 129);
            ctx.fillStyle = "rgba(" + color.r + "," + color.g + "," + color.b + "," + color.a + ")";
